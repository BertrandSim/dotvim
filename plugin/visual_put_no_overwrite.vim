" Adapted from Luc Hermitte's repl-visual-no-reg-overwrite.vim
" Modified RestoreRegister() to work _correctly_ with clipboard=unnamed,unnamedplus

" For an alternative, fuller plugin, see Ingo Karkat's vim-ReplaceWithRegister
...
function! RestoreRegister()
  if &clipboard =~ '\m\<unnamed\>'
    let @* = s:restore_reg
  endif
  if &clipboard =~ '\m\<unnamedplus\>'
    let @+ = s:restore_reg
  endif
  let @" = s:restore_reg
  return ''
